...
def test_array_api():
    ARRAY_API_TESTS_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), os.environ.get("ARRAY_API_TESTS_DIR", "../array-api-tests")))
    ARRAY_API_TESTS_REV = os.environ.get("ARRAY_API_TESTS_REV", "c48410f96fc58e02eea844e6b7f6cc01680f77ce")
    ARRAY_API_TESTS_SKIPS = os.path.abspath(os.path.join(os.path.dirname(__file__), os.environ.get("ARRAY_API_TESTS_SKIPS", "array-api-skips.txt")))
    ...
    subprocess.run(
        ["git", "--git-dir", f"{ARRAY_API_TESTS_DIR}/.git", 
        "--work-tree", ARRAY_API_TESTS_DIR, "clean", "-xddf"],
        check=True
    )
    ...
    subprocess.run(
        ["git", "--git-dir", f"{ARRAY_API_TESTS_DIR}/.git", 
        "--work-tree", ARRAY_API_TESTS_DIR, "fetch"],
        check=True
    )
    ...
    subprocess.run(
        ["git", "--git-dir", f"{ARRAY_API_TESTS_DIR}/.git", 
        "--work-tree", ARRAY_API_TESTS_DIR, "reset", "--hard", ARRAY_API_TESTS_REV],
        check=True
    )
    ...
    result = subprocess.run(
        [
            ...
            *ARRAY_API_TESTS_ARGS.split(),
            ...
            "--disable-deadline",
            "--disable-warnings",
            "--skips-file", ARRAY_API_TESTS_SKIPS
        ],
