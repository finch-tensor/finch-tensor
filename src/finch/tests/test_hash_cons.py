...
from finch.symbolic.hashcons import _table
...
def test_interning_is_weak():
    ...
    key = (type(t), *t.__hash_keys__())
    assert _table[key] is t
    del t
    gc.collect()
    assert key not in _table


def test_interned_across_hierarchies():
    assert asm.Variable("x", ftype(np.int64)) is asm.Variable("x", ftype(np.int64))
    assert asm.Variable("x", ftype(np.int64)) is not asm.Variable(
        "x", ftype(np.float64)
    )
