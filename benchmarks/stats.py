...
import finch as fl
from finch import ffuncs as ffunc
...
def plot(errors):
    ...
    for i in range(n_stats):
        ax.axvspan(i - 0.5, i + 0.5, color=band_colors[i], alpha=0.4, zorder=0)

    for i, m_type in enumerate(MATRIX_TYPES):
        vals = [errors[stat][i] for stat in stats]
        offset = (i - n_types / 2 + 0.5) * bar_width

        ax.bar(x + offset, vals, width=bar_width, label=m_type, color=simple_colors[i])
